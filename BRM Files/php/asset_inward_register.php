...
if($_GET['action']== 'add') {
    ...
    $get_received_status = $db->qry("SELECT count(apo_id) AS apo_id FROM assets_podata WHERE asset_po_id = " . $asset_inward_po_id . " AND apo_qty_received <> apo_qty");

    if((int) $get_received_status[0]['apo_id'] == 0) {
        $update_po_status = $db->qry("UPDATE assets_pos SET assetpo_status = 'Received' WHERE assetpo_id = " . $asset_inward_po_id);
    }
